// menu style
const menuListStyle = `md:ml-10 text-base text-gray-900 leading-snug text-surface hover:text-tertiary font-medium capitalize transform duration-200`;

// mobile menu style
const mobileMenuStyle = `m-0 block py-4 px-6`;

// blog style과 notebook style
const posth1Style = `text-[40px] font-bold mb-4 mt-6 border-b border-gray-300 pb-2.5`;
const posth2Style = `text-[32px] font-bold mb-4 mt-6 border-b border-gray-300 pb-2.5`;
const posth3Style = `text-[28px] font-bold mb-4 mt-6 border-b border-gray-300 pb-2.5`;
...
const postaStyle = `text-lg text-primaryLight underline transition duration-200 hover:text-primaryDark`;
...
const postliStyle = `list-none relative mb-2 leading-relaxed tracking-wide text-justify before:content-[''] before:absolute before:left-0 before:top-1/2 before:-translate-y-1/2 before:w-[3px] before:h-[calc(100%+4px)] before:bg-[#70A8A9] pl-4`;
...
const postthStyle = `overflow-auto bg-bglv1 border border-gray-600 px-4 py-2.5 font-medium text-sm capitalize whitespace-nowrap`;
const posttbodyStyle = `text-left`;
const posttdStyle = `border border-gray-600 px-4 py-2.5 text-sm break-keep`;

const posthrStyle = `my-4 border-gray-400 border-2 rounded-lg`;
const postemStyle = `text-lg font-medium italic pr-0.5`;
const poststrongStyle = `text-lg font-bold text-primaryDark`;

// blog에 최상단 제목과 이미지 날짜 카테고리를 표시하는 부분
const postcategoryStyle = `bg-tertiary text-white text-sm font-medium px-3 py-1.5 rounded-lg tracking-wide transition duration-200 ease-in-out hover:bg-secondary cursor-pointer`;
const posttitleStyle = `md:text-[40px] md:leading-[56px] text-[32px] leading-[40px] font-bold my-3 text-primaryDark`;
...
const postauthorStyle = `inline text-sm font-semibold text-gray-800 mr-2`;
...
// bloglist 목록 스타일
const bloglistFirstCardStyle = `lg:col-span-3 md:col-span-2 col-span-1 h-auto rounded-2xl overflow-hidden bg-bglv1 transition duration-200 ease-in-out transform hover:-translate-y-1 hover:scale-105 flex md:flex-row flex-col flex-1 cursor-pointer p-4`;
...
const bloglistFirstCardDescriptionStyle = `text-gray-800 text-base font-normal leading-snug md:max-h-40 md:line-clamp-[7] line-clamp-3 mb-3`;

const bloglistCardStyle = `lg:max-w-sm overflow-hidden bg-bglv1 transition duration-200 ease-in-out transform hover:-translate-y-1 hover:scale-105 cursor-poitner col-span-1 w-auto cursor-pointer p-4 rounded-2xl`;
const bloglistCardImgStyle = `w-full h-[200px] object-cover object-center rounded-2xl overflow-hidden`;

const bloglistCardBodyStyle = `py-4 pr-4`;
const bloglistCardTitleStyle = `font-bold text-2xl text-primaryDark mb-3`;
const bloglistCardCategoryStyle = `inline-block bg-tertiary text-white md:text-sm font-medium mb-3 px-3 py-1.5 rounded-lg transition duration-200 ease-in-out hover:bg-secondary`;
const bloglistCardDescriptionStyle = `text-base font-normal leading-snug h-16 line-clamp-3 mb-4`;
